...
export default async function HeroSection() {
  ...
  const res = await fetch(
    `${process.env.BASE_API}/HomeContent/GetPagedHomeContentList?pageIndex=1&pageSize=10`
  );
      ...
      <div className='hero-section'>
        <div className="relative z-[8] mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="mx-auto mt-[110px]">
            ...
            <HeadingXL label={item?.topic} />
            <Para label={item?.description} />

            <div className="mt-8">
              <Link href="/login">
                <button className="bg-white text-green-800 font-semibold px-6 py-3 rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50 transition-colors duration-300">
                  Reserve Access
                </button>
              </Link>
            </div>
